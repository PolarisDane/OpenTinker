...
class SchedulerClientLifecycleManager:
    ...
    def start_reward_server(
        ...
        if not os.path.isabs(reward_server_path):
            ...
            reward_server_path = os.path.normpath(
                os.path.join(current_dir, reward_server_path)
            )
        ...
        try:
            response = requests.get(
                f"http://{reward_ip}:{reward_port}/health", timeout=2
            )
        ...
        if self._scheduler_client and self._current_job_id:
            try:
                ...
                try:
                    ...
                    print(
                        f"✓ Job {self._current_job_id} cancelled: {result.get('message', 'Success')}"
                    )
                except Exception as cancel_error:
                    # If cancel fails (e.g., job already completed), try to complete it
                    print(
                        f"⚠ Could not cancel job (might be already completed): {cancel_error}"
                    )
                    try:
