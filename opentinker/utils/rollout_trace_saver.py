...
def init_weave_tracing(
    ...
    try:
        ...
        if RolloutTraceConfig.get_backend() is not None:
            print(
                f"Tracing already initialized with backend: {RolloutTraceConfig.get_backend()}"
            )
            return True
        ...
        print(
            f"✓ Weave tracing initialized: project={project_name}, experiment={experiment_name}"
        )
        return True
...
def init_mlflow_tracing(
    ...
    try:
        ...
        print(
            f"✓ MLflow tracing initialized: project={project_name}, experiment={experiment_name}"
        )
        return True
...
def init_global_saver(
    output_dir: str = "/tmp/rollout_traces", **kwargs
) -> RolloutTraceSaver:
