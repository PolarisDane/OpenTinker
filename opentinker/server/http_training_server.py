...
def serialize_dataproto(data: DataProto) -> Dict[str, Any]:
    ...
    for k, v in data.non_tensor_batch.items():
        if isinstance(v, np.ndarray):
            ...
            if v.dtype == object:
                # Use tolist() instead of flatten().tolist() to preserve nested structure
                # e.g., array of lists [[msg1, msg2], [msg1, msg2]] stays as nested lists
                data_list = v.tolist()
                serialized_non_tensor[k] = {
                    "__type__": "numpy.ndarray",
                    "__dtype__": "object",
                    "__shape__": list(v.shape),
                    "__data__": data_list,  # Keep nested structure (lists of dicts, etc.)
                }
            else:
