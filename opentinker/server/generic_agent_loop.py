...
class GenericAgentLoop(AgentLoopBase):
    ...
    def init_class(cls, config, tokenizer, processor, **kwargs):
        ...
        cls.max_tokens_per_turn = config.actor_rollout_ref.rollout.multi_turn.get("max_tokens_per_turn", None)
        ...
        interaction_config_content = config.actor_rollout_ref.rollout.multi_turn.get("interaction_config_content", None)
        
        if interaction_config_content:
            import tempfile
            # Create a local temp file with the content on THIS worker's node
            fd, local_path = tempfile.mkstemp(suffix='.yaml', prefix='interaction_config_worker_')
            with os.fdopen(fd, 'w') as f:
                f.write(interaction_config_content)
            cls.interaction_config_file = local_path
            print(f"[GenericAgentLoop] Created local interaction config from content: {local_path}")
        ...
        if cls._trace_output_dir:
            ...
            print(f"[GenericAgentLoop] Rollout trace saving ENABLED: {cls._trace_output_dir} (PID: {cls._process_id})")
        else:
            cls._save_traces = False
            print(f"[GenericAgentLoop] Rollout trace saving DISABLED (set ROLLOUT_TRACE_DIR to enable)")
        ...
        if weave_project is None:
            weave_project = config.actor_rollout_ref.rollout.multi_turn.get("weave_project", None)
        
        if weave_project:
            try:
                from verl.utils.rollout_trace import RolloutTraceConfig
                experiment_name = config.actor_rollout_ref.rollout.multi_turn.get("experiment_name", "default")
                ...
                print(f"[GenericAgentLoop] Weave tracing ENABLED: project={weave_project}, experiment={experiment_name}")
            except ImportError:
                print(f"[GenericAgentLoop] WARNING: Weave not installed (pip install weave)")
        ...
        if self.interaction_config_file:
            ...
            if "name" not in interaction_kwargs:
                ...
                if self.interaction_map:
                    ...
                    logger.info(f"Using default interaction: {default_interaction_name}")
                else:
                    raise ValueError("No interactions configured in interaction_config_file")
            ...
            if interaction_name == "gomoku":  # Only for Gomoku
                if hasattr(interaction, '_instance_dict') and request_id in interaction._instance_dict:
                    initial_board_state = interaction._instance_dict[request_id].get('initial_board_state')
    ...
    async def _handle_generating_state(
        ...
        if self.max_tokens_per_turn and len(response_token_ids) > self.max_tokens_per_turn:
            logger.debug(f"Truncating turn response from {len(response_token_ids)} to {self.max_tokens_per_turn} tokens")
        ...
        agent_data.response_mask += [1] * len(agent_data.response_ids)  # mask=1 for LLM tokens
    ...
    async def _handle_interacting_state(self, agent_data: GenericAgentData) -> GenericAgentState:
    ...
    async def _save_rollout_trace(
        ...
        try:
            ...
            prompt_text = await self.loop.run_in_executor(
                None, 
                lambda: self.tokenizer.decode(output.prompt_ids, skip_special_tokens=True)
            )
            response_text = await self.loop.run_in_executor(
                None,
                lambda: self.tokenizer.decode(output.response_ids, skip_special_tokens=True)
            )
            
            # Extract per-turn board states from messages
            per_turn_board_states = self._extract_per_turn_board_states(agent_data.messages, agent_data)
            ...
            trace_data = {
                ...
                "initial_prompt": agent_data.messages[0] if agent_data.messages else None,
                ...
                "response_mask_analysis": {
                    ...
                    "env_tokens": len(output.response_mask) - sum(output.response_mask),  # mask=0
                    "llm_ratio": sum(output.response_mask) / len(output.response_mask) if output.response_mask else 0,
                },
            ...
            await self.loop.run_in_executor(
                None,
                lambda: trace_file.write_text(json.dumps(trace_data, indent=2, default=str))
            )
            ...
            await self.loop.run_in_executor(
                None,
                lambda: (
                    jsonl_file.open('a').write(json.dumps(trace_data, default=str) + '\n')
                )
            )
    ...
    def _extract_per_turn_board_states(
        ...
        for i, message in enumerate(messages):
            ...
            if role == "system" and initial_board_state:
                ...
                structured_board = env_info.get("board_state") if isinstance(env_info, dict) else None
                env_info_idx += 1
            ...
            if visual_board and structured_board:
                ...
                verification_status = "match" if visual_normalized == expected_normalized else "mismatch"
            ...
            turn_info = {
                ...
                "message_content_preview": content[:300] + "..." if len(content) > 300 else content,
            }
