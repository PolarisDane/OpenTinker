...
class StaticDatasetGeneratorVL(StaticDatasetGenerator):
    ...
    def filter_overlong_samples(
        ...
        print(f"\n{'='*60}")
        print(f"[VL Pre-Filter] Starting overlong sample filtering...")
        ...
        for idx in range(original_count):
            ...
            if length <= max_prompt_length:
                ...
                if len(removed_lengths) <= 5:  # Log first 5 removed samples
                    print(f"[VL Pre-Filter]   Removed sample {idx}: length={length} > {max_prompt_length}")
        ...
        print(f"[VL Pre-Filter] Filtering complete!")
        print(f"[VL Pre-Filter]   Original samples: {original_count}")
        print(f"[VL Pre-Filter]   Removed samples:  {removed_count} ({removed_ratio*100:.1f}%)")
        ...
        if removed_ratio > 0.2:
            print(f"[VL Pre-Filter] ⚠️  WARNING: High filtering ratio ({removed_ratio*100:.1f}%)!")
            print(f"[VL Pre-Filter]    Consider increasing max_prompt_length in config.")
