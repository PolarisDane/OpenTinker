...
class BaseGameStats:
    ...
    def register_game_start(self, instance_id: str, job_id: str = "default"):
        ...
        with self._lock:
            if instance_id not in self._started_games:
                ...
                print(f"[DEBUG] Game registered on reset: {instance_id} (job_id={job_id}, total: {len(self._started_games)})")
    ...
    def record_game_result(self, info: Dict[str, Any], reward: float, done: bool, instance_id: str = "default", job_id: str = "default"):
        ...
        with self._lock:
            ...
            if instance_id not in self._started_games:
                self._started_games.add(instance_id)
                print(f"[DEBUG] New game started on step: {instance_id} (job_id={job_id}, total: {len(self._started_games)})")
    ...
    def reset_step_stats(self):
        ...
        with self._lock:
            ...
            # Debug logging
            print(f"[DEBUG] reset_step_stats called: step {self._current_step} -> {self._current_step + 1}, "
            ...
            return {"step": self._current_step, "message": "Step stats reset", "incomplete_cleared": incomplete_count}
...
class MultiJobGameStats:
    ...
    def reset_all_jobs(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {"message": f"All stats reset for {job_count} jobs", "jobs_cleared": job_count}
...
def create_game_app(
    ...
    **game_kwargs
    ...
    async def step(request: StepRequest):
        ...
        stats = multi_stats.get_job_stats(job_id)
        stats.record_game_result(result.info, result.reward, result.done, instance_id, job_id)
