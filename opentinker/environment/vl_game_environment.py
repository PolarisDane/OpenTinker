...
from opentinker.environment.base_data_generator_vl import DynamicGameDatasetVL, collate_fn_vl
...
class VLGameEnvironment(GameEnvironment):
    ...
    def _setup_dataloader(self):
        ...
        processor_path = getattr(self.config, 'processor_path', self.config.tokenizer_path)
        ...
        virtual_size = (
            num_steps * batch_size if num_steps 
        ...
        if self.val_data_paths:
            ...
            logger.info(f"Validation dataloader: {val_batch_size} fixed samples in {len(self.val_dataloader)} batch(es)")
