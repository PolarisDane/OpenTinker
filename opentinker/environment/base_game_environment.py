...
class GameEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        if hasattr(config, 'interaction') and hasattr(config.interaction, 'config'):
            ...
            self.env_endpoint = interaction_config.get('env_endpoint', 'http://localhost:8081')
            self.max_steps = interaction_config.get('max_steps', 100)
            self.observation_template = interaction_config.get('observation_template', '{observation}')
            ...
            self.observation_template = config.get('observation_template', '{observation}')
    ...
    def _setup_dataloader(self):
        ...
        virtual_size = num_steps * self.config.batch_size if num_steps else self.config.batch_size * 100
    ...
    def _setup_interaction_config(self):
        ...
        fd, path = tempfile.mkstemp(suffix='.yaml', prefix=f'{self.interaction_name}_interaction_config_')
