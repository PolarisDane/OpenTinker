...
class GameStatsClient:
    ...
    def __init__(
        self, game_server_url: str, job_id: str = "default", timeout: float = 5.0
    ):
    ...
    def get_all_stats(self) -> Dict[str, Any]:
        ...
        """
        response = requests.get(
            f"{self.url}/stats", params={"job_id": self.job_id}, timeout=self.timeout
        )
        response.raise_for_status()
        return response.json()
    ...
    def list_jobs(self) -> Dict[str, Any]:
        ...
        """
        response = requests.get(f"{self.url}/stats/jobs", timeout=self.timeout)
        response.raise_for_status()
        return response.json()
