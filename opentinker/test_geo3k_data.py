...
def test_geo3k_data(data_path: str, num_samples: int = 5):
    ...
    for i in range(min(num_samples, len(dataset))):
        ...
        if image_keys:
            ...
            print(f"   - No image tensors found")
        
        print(f"   - data_source: {sample.get('data_source')}")
        print(f"   - interaction_kwargs: {sample.get('interaction_kwargs', {}).get('name')}")
...
def main():
    ...
    if not os.path.exists(data_path):
        ...
        print(f"  python verl/examples/data_preprocess/geo3k.py --local_save_dir ~/data/geo3k")
        return False
